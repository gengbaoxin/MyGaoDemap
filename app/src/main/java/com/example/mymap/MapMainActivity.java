                ...
                try {
                    if (null != loc) {
                        ...
                        if (loc.getErrorCode() == 0) {//可在其中解析amapLocation获取相应内容。
                            location = loc;
                            SPUtils.putString(MapMainActivity.this, DatasKey.LOCATION_INFO, gson.toJson(location));
...
//                        Toast.makeText(MainActivity.this, "搜索", Toast.LENGTH_SHORT).show();
                        startActivityForResult(new Intent(MapMainActivity.this, SearchActivity.class), SEARCHREQUESTCODE);
                        break;
...
//                        Toast.makeText(MainActivity.this, "定位", Toast.LENGTH_SHORT).show();
                    ...
                    case R.id.bt_send:
                        if (null != mList && 0 < mList.size() && null != mAddressAdapter) {
                            ...
                            Toast.makeText(MapMainActivity.this, "发送：" + poiItem.getTitle() + "  " + poiItem.getSnippet() + "  " + "纬度：" + poiItem.getLatLonPoint().getLatitude() + "  " + "经度：" + poiItem.getLatLonPoint().getLongitude(), Toast.LENGTH_SHORT).show();
                        }
                        break;
                }
            }
